...
export default function EditEvent() {
  ...
  useEffect(() => {
    api
      ...
      .then((res) => {
        ...
        const date = e.date ? new Date(e.date).toISOString().slice(0, 16) : '';
        setForm({
          title: e.title || '',
          ...
          images: e.images || [],
          ticketTypes: (e.ticketTypes?.length ? e.ticketTypes : [{ name: 'General', price: 0, quantity: 100 }]).map((t) => ({
            name: t.name,
            price: t.price,
            quantity: t.quantity,
          })),
  ...
  const updateForm = (key, value) => setForm((f) => (f ? { ...f, [key]: value } : f));
  ...
  const addTicketType = () => {
    setForm((f) => (f ? { ...f, ticketTypes: [...f.ticketTypes, { name: '', price: 0, quantity: 0 }] } : f));
  };
  const removeTicketType = (index) => {
    setForm((f) => (f ? { ...f, ticketTypes: f.ticketTypes.filter((_, i) => i !== index) } : f));
  };

  const handleImageSelect = (e) => {
    const files = Array.from(e.target.files);
    const currentTotal = (form?.images?.length || 0) + selectedImages.length;
  ...
  const removeImage = (index) => {
    if (index < imagePreviews.length) {
      ...
      const existingIndex = index - imagePreviews.length;
      setForm((f) =>
        f
          ? {
              ...f,
              images: f.images.filter((_, i) => i !== existingIndex),
            }
          : f
      );
    }
      ...
      <form onSubmit={handleSubmit} className="mt-6 card p-6 space-y-4">
        ...
        <div className="border-t border-gray-200 pt-4">
          ...
          <input
            ...
            disabled={uploadingImages || (form.images?.length || 0) + selectedImages.length >= 10}
          />
          {(imagePreviews.length > 0 || (form.images?.length || 0) > 0) && (
        ...
        <div className="flex gap-3 pt-4">
          ...
          <button type="submit" disabled={saving || uploadingImages} className="btn-primary flex-1">
